...
func upddate_gui() -> void:
	...
	$Menu.reset_size()
	$startGameButton.reset_size()
	$settingsButton.reset_size()
	$Settings/Settings.reset_size()
	$Settings/FPS/Label.reset_size()
	$Settings/FPS/CheckButton.reset_size()
	$Settings/Difficulty/Label.reset_size()
	$Settings/Difficulty/Button.reset_size()
	$Settings/Language/Label.reset_size()
	$Settings/Language/languageButton.reset_size()
	$credits.reset_size()
	$credits/Panel/credits.reset_size()
	$credits/Panel/creditsLabel.reset_size()
	$"Continue Game".reset_size()
	...
	$Settings/Language/languageButton.position = Vector2($Settings.size.x - $Settings/Language/languageButton.size.x - 20, $Settings/Language/Label.position.y)
